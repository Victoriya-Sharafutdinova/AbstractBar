...
namespace AbstractGarmentFactoryView
{
    public static class MailCustomer
    {
        ...
        private static void GetMessages(int number)
        {
            ...
            string indentSubjectMessage = string.Empty;
            string indentBodyMessage = string.Empty;
            ...
            while (true)
            {
                ...
                if (response.Length > 4)
                {
                    ...
                    {
                        indentSubjectMessage = GetSubject(ref response, ref coding);

                        indentBodyMessage = GetBody(response, coding);
                    }
                    if (!string.IsNullOrEmpty(messageId) && !string.IsNullOrEmpty(from) &&         
                        !string.IsNullOrEmpty(indentSubjectMessage) && !string.IsNullOrEmpty(date))
                    {
                        APICustomer.PostRequest<MessageInfoBindingModel, bool>("api/MessageInfo/AddElement",
                        new MessageInfoBindingModel
                        {
                            MessageId = messageId,
                            FromMailAddress = from,
                            DateDelivery = Convert.ToDateTime(date),
                            Subject = indentSubjectMessage,
                            Body = indentBodyMessage
                        }); 
 
                        messageId = string.Empty;
                        from = string.Empty;
                        date = string.Empty;
                        indentSubjectMessage = string.Empty;
                        indentBodyMessage = string.Empty;
                    }
                }
            }
        }
